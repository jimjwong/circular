...
import { ArrowLeft, CalendarDays, Clock3, MapPin, Trash2, UserRoundX, Users } from "lucide-react";
...
export default async function EventDetailPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const [{ data: event }, { data: spaces }, { data: registrations }] = await Promise.all([
    supabase.from("events").select("id, tenant_id, space_id, host_id, title, description, starts_at, ends_at, location_type, location_url, capacity, status").eq("id", eventId).eq("tenant_id", organization.id).maybeSingle(),
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_330px]">
        <div className="space-y-5">
          <section className="rounded-[24px] bg-[#183f30] p-7 text-white sm:p-9">
        ...
        <aside className="space-y-5">
          <section className="rounded-[22px] border border-[#e0e7e2] bg-white p-5"><div className="flex items-center gap-2"><Users size={16} className="text-[#317657]"/><h2 className="font-display font-bold">Attendance</h2></div><p className="mt-3 text-3xl font-bold">{going.length}<span className="text-sm font-medium text-[#829087]">{event.capacity ? ` / ${event.capacity}` : " going"}</span></p>{registrationOpen && full && !registered ? <button disabled className="mt-5 h-11 w-full cursor-not-allowed rounded-xl bg-[#edf1ee] text-xs font-semibold text-[#7b8981]">Event is full</button> : registrationOpen && <form action={toggleEventRegistration} className="mt-5"><input type="hidden" name="eventId" value={event.id}/><SubmitButton className={`h-11 w-full rounded-xl text-xs font-semibold ${registered ? "bg-[#edf1ee] text-[#53675b]" : "bg-[#183f30] text-white"}`}>{registered ? "Cancel registration" : "Register for event"}</SubmitButton></form>}{!registrationOpen && <p className="mt-4 rounded-xl bg-[#f3f5f3] p-3 text-xs text-[#76847c]">Registration is closed.</p>}</section>
