...
function SettingsScreen({ tenant, notify }: { tenant: string; notify: (message: string) => void }) {
  ...
  const selectSection=(next:string)=>{ if(next==="Roles & permissions") return router.push("/team"); if(next==="Themes & branding") return router.push("/settings/appearance"); setSection(next); };
