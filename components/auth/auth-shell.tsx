...
export function AuthShell({ eyebrow, title, description, children }: { eyebrow: string; title: string; description: string; children: React.ReactNode }) {
  return <main className="grid min-h-screen bg-[#f5f7f5] lg:grid-cols-[1.08fr_.92fr]">
    <section className="relative hidden overflow-hidden bg-[#173f31] p-12 text-white lg:flex lg:flex-col">
      ...
      <div className="relative flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-white font-display font-bold text-[#173f31]">C</span><span><b className="font-display block">Commune</b><small className="text-[#b9cec4]">Community operating system</small></span></div>
    ...
    <section className="flex items-center justify-center p-5 sm:p-10"><div className="w-full max-w-[420px]"><div className="mb-8 flex items-center gap-3 lg:hidden"><span className="grid size-9 place-items-center rounded-xl bg-[#173f31] font-display text-sm font-bold text-white">C</span><b className="font-display">Commune</b></div><span className="text-[10px] font-bold uppercase tracking-[.16em] text-[#347457]">{eyebrow}</span><h1 className="font-display mt-2 text-3xl font-bold tracking-[-.04em] text-[#1b2e24]">{title}</h1><p className="mt-3 text-sm leading-6 text-[#718078]">{description}</p>{children}</div></section>
  </main>;
}
