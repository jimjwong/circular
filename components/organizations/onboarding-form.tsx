...
export function OnboardingForm() {
  ...
  return <form action={action} className="mt-8 space-y-5">
    ...
    <label className="block"><span className="mb-2 block text-xs font-semibold">Workspace URL</span><div className="flex h-11 overflow-hidden rounded-xl border border-[#dbe3de] focus-within:border-[#73aa90] focus-within:ring-2 focus-within:ring-[#dceee5]"><span className="grid place-items-center bg-[#f2f5f3] px-3 text-xs text-[#7f8d85]">commune.local/</span><input required name="slug" key={suggestedSlug} defaultValue={suggestedSlug} placeholder="acme-community" className="min-w-0 flex-1 px-3 text-sm outline-none"/></div>{state?.errors?.slug?.[0]&&<span className="mt-1 block text-xs text-[#ae523e]">{state.errors.slug[0]}</span>}</label>
