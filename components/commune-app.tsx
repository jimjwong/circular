...
import {
  Activity, BarChart3, Bell, Bot, CalendarDays, Check,
  ...
  Hash, Heart, Inbox, LayoutDashboard, LockKeyhole,
...
function navigationHref(view: View) {
  ...
  if (view === "members") return "/members" as const;
  if (view === "email") return "/email" as const;
  if (view === "website") return "/website" as const;
...
function SpacesScreen({ openCommunity, openTeam, notify }: { openCommunity: () => void; openTeam: () => void; notify: (message: string) => void }) {
  return <div className="grid gap-5 xl:grid-cols-[1fr_300px] animate-enter">
    ...
    <div className="space-y-5">
      ...
      <Card className="overflow-hidden bg-[#183f30] p-5 text-white"><Sparkles size={20} className="text-[#f3c477]"/><h3 className="font-display mt-4 font-bold">Design with AI</h3><p className="mt-2 text-xs leading-5 text-[#c7d8cf]">Describe your community and Commune will suggest spaces, access rules, and an onboarding flow.</p><button type="button" onClick={()=>notify("AI space builder will be added in a later phase")} className="mt-5 text-xs font-bold text-[#f3c477]">Open builder →</button></Card>
    </div>
  </div>;
}
...
function MemberPreview({ onExit }: { onExit: () => void }) {
  ...
  return <div className="min-h-screen bg-[#f5f3ee] text-[#1d2c25]">
    <header className="sticky top-0 z-20 border-b border-[#e4e2dc] bg-[#f5f3ee]/90 px-4 backdrop-blur-xl sm:px-8"><div className="mx-auto flex h-16 max-w-6xl items-center gap-5"><div className="grid size-9 place-items-center rounded-xl bg-[#183f30] text-sm font-bold text-white">C</div><b className="font-display">The Collective</b><nav className="ml-5 hidden gap-5 text-xs font-semibold text-[#637168] md:flex"><button type="button" onClick={()=>router.push("/community")}>Home</button><button type="button" onClick={()=>router.push("/community")}>Spaces</button><button type="button" onClick={onExit}>Events</button><button type="button" onClick={onExit}>Courses</button><button type="button" onClick={()=>router.push("/members")}>Members</button></nav><div className="ml-auto flex gap-2"><IconButton label="Search" onClick={onExit}><Search size={16}/></IconButton><IconButton label="Notifications" onClick={()=>router.push("/notifications")}><Bell size={16}/></IconButton><Button secondary onClick={onExit}>Admin <ExternalLink size={13}/></Button></div></div></header>
...
export function CommuneApp({ organizations = demoOrganizations, activeOrganizationId, currentUser = demoUser, initialView = "overview", grantedPermissions = ["workspace.full_access", ...Object.values(DASHBOARD_VIEW_PERMISSIONS).flat()], onSwitchOrganization }: { organizations?: OrganizationSummary[]; activeOrganizationId?: string; currentUser?: CurrentUser; initialView?: string; grantedPermissions?: string[]; onSwitchOrganization?: (tenantId: string) => Promise<void> }) {
  ...
  const runPrimaryAction=()=>{
    ...
    if (view === "email") return router.push("/email");
    if (view === "website") return router.push("/website");
    if (view === "settings") return router.push("/settings/general");
    setCreateOpen(true);
  };
